...
""" + json.dumps(self.mytools.all_tools)
    ...
    def execute_function(self, function_name, **kwargs):
        '''Execute a function from the loaded toolfile by its name with given parameters.'''
        ret = ''
        mytools = self.load_toolfile()
        try:
            ret = getattr(mytools, function_name)(**kwargs)
        except TypeError as e:
            ret = f"Error executing function '{function_name}': {str(e)}"
        return ret
