...
def extract_failure_reports(filename):
    with open(filename, 'r') as f:
        content = f.read()

    # Match the section between "Failure Reports/Log Files:" and the next blank line or dashed line
    match = re.search(r'Failure Reports/Log Files:\n[-]+\n(.*?)(?:\n\n|GKutils|GK Report|$)', content, re.DOTALL)
    if not match:
        print("No 'Failure Reports/Log Files' section found.")
        return []

    section = match.group(1)

    # Extract all file paths (anything after "-- " until newline)
    log_files = re.findall(r'--\s*(\S[^\n]*)', section)

    return [log.strip() for log in log_files]

def create_input_file_for_ask_batch(errors, inputfile, sectionname, mode='a'):
    ''' 
    ...
    '''
    LOGGER.info(f"Creating input file for ask_batch.py at: {inputfile}, section: {sectionname}, mode: {mode}")
    with open(inputfile, mode) as f:
        for idx, error in enumerate(errors, start=1):
            # skip if error is less than 4 words
            if len(error.split()) < 4:
                LOGGER.info(f"Skipping error (less than 4 words): {error}")
                continue
            f.write(f"[Q{idx}:{sectionname}]\n")
            f.write(f"{error}\n\n")
...
def main(args):
    ...
    logfiles = extract_failure_reports(os.path.join(args.workarea, 'GATEKEEPER', 'gk_report.txt'))
    LOGGER.info(f"Error/Report logfiles: {pformat(logfiles)}")

    
    # Create input file for ask_batch.py
    hostname = os.uname()[1]
    pid = os.getpid()
    username = os.environ['USER']
    inputfile = f'/tmp/ask_batch_input_{username}_{hostname}_{pid}.txt'
    outputfile = f'/tmp/ask_batch_output_{username}_{hostname}_{pid}.txt'
    if args.outputfile:
        outputfile = args.outputfile
    for i, logfile in enumerate(logfiles):
        LOGGER.info(f"Processing logfile: {logfile}")
        cmd = f"""grep -i -A 2 error {logfile} | ask.py -q 'Help me summarize the following errors which was extracted from a logfile, so that it is suitable to be sent for a similarity search from a vector db. If there are similar errors which are closely related, just report it once. If there are errors which makes no sense, omit it. Omit errors which is make/makefile related. Report the errors in a json array, like this: ["error 1 ...", "error 2 ..."]. Do not provide any explanation.' --pipe --promptboost_disable -sm """
        output = subprocess.getoutput(cmd)
        output = remove_newline_and_markdown(output)
        errors = json.loads(output)  # just to confirm it's valid json

        mode = 'a' if i else 'w'
        sectionname = os.path.basename(logfile)
        create_input_file_for_ask_batch(errors, inputfile, sectionname, mode=mode)


    # Call ask_batch.py with the input file
    LOGGER.info(f"Calling ask_batch.py with input file: {inputfile}")
    cmd = f"""ask_batch.py -t 5 -i {inputfile} -o {outputfile} -l /nfs/site/disks/da_scratch_1/users/yltan/depot/da/infra/genai/main/faissdbs/psgcth2tfm/default/"""
    os.system(cmd)
...
{examples()}
    ...
    parser.add_argument('--workarea', required=True, default=None, help='Fullpath Of Workarea directory')
    parser.add_argument('--outputfile', default=None, required=False, help='Output file to store the final response')
