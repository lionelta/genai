...
def main(args):
    ...
    b = BaseAgent()
    toolagent = ToolAgent()
    toolagent.toolfile = os.path.join(rootdir, 'toolfiles', 'ask_pdd_toolfile.py')
    ...
    if args.no_llm:
        print("LLM processing is disabled. Exiting.")
        return False
    res = b.run()
...
def orchestrator_prompt(user_query, available_functions):
    ...
    There are two types of questions you can answer:
    1. Questions that do not need to know subystem (can be abbreviated SS) and block information.
    2. Questions that need to know subsystem and block information.

    The API functions require a subsystem and a block.  User query may not explicitly mention these, so you need to infer them based on previous queries if it's not specified.
    If subsystem and blocks are not provided, no planning needed, just call llm to answer the query directly.
    Users may also use wildcards or regular expressions to specify multiple subsystems, blocks, or metrics. 
    When user is requesting status or metrics for metrics, any subset match is acceptable.  
    For example, if user requests metrics or status for power, "Cell Leakage Power" and "Total Dynamic Power" are acceptable.  
    If user did not specify which flow step to provide metrics, 
    Show metrics for route_opt if user did not which step to get metrics.  
    If a user wants data for a specific step, let that show up only when specified with either an actual step name or all steps in the prompt.
...
def examples():
    examples = """
- what is the health status of the PDD system?
- show me the executive summary
- what version of PDD is currently running?
- list all users in the PDD system
- list all subsystems available in PDD
- what blocks are available in the HSIO subsystem?
- what are all the experiment tags for the HSIO subsystem?
- what experiment tags are available for the ioss3_ddr5_mc_sf block in the HSIO subsystem?
- get me the list of all users and their last login times
- what is the experiment data of SS noc?
- what is the setup and hold times for all the blocks in HSIO
- please provide full details of ioss3_lpddr_mc_sf
- which block in HSIO has missing DRC violations
"""
    return examples
...
{examples()}
    ...
    parser = argparse.ArgumentParser(prog='ask_pdd.py', formatter_class=MyFormatter, epilog=epilog)
    ...
    parser.add_argument('--no_llm', action='store_true', default=False, help='Disable LLM processing.')
